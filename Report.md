...
- Designed executes one instruction per clock cycle.
...
5. **Register File**:
   ...
   - **Data Flow**: Receives register addresses (`A1`, `A2`, `A3`), writes data (`WD3`), and outputs register data (`RD1`, `RD2`) to the `ALU` and `Data Memory`.
...
C code - Computing the sum of N natural numbers.
